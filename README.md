...
- **탐색 릴레이**: Claude가 조사하면 GPT가 이어받아 빈틈을 채우고, 흐름에 맞지 않는 자료는 이유와 함께 빼고, 유망한 분야를 더 파고듭니다. 번갈아 반복합니다.
- **프로젝트 관리**: 실행 중 **중지**, **이름 변경**, **삭제**, 사이드바 검색, 모델을 바꿔 **이어서 실행**, 멈춘 연구에도 후속 질문.
- **지금 상황 한눈에**: 누가 어떤 단계를 몇 분째 하는지, 총 시간·호출·토큰, 요청마다 타임스탬프, 작업자(worker) 상태 경고.
...
- **PDF 첨부**: 텍스트가 있는 PDF를 올리면 브라우저에서 쪽 번호와 함께 텍스트를 뽑아 참고 자료로 넣습니다.
- **기록 보존**: 모든 답변·문서 버전·대화가 호출마다 로컬 파일에 저장됩니다. 새로고침이나 재시작 후에도 보던 프로젝트·탭과 쓰던 입력 내용이 그대로 열립니다.
...
7. (선택) 참고 텍스트를 붙여넣거나 PDF, TXT, MD, CSV, JSON, LOG 파일을 첨부합니다. 입력칸의 **미리보기**로 Markdown 모양을 확인할 수 있습니다.
...
| "응답 시간 제한"으로 실패 | 조사·초안·수정 10분, 합치기·보고서 7분, 나머지 5분 제한입니다. 자동으로 2회 재시도하고, 그래도 실패하면 effort를 낮추거나 `.env.local`에 `CLI_TIMEOUT_SECONDS=900`을 넣고 **이어서 실행** |
| "과부하나 네트워크 문제" | 서비스가 일시적으로 바쁜 경우입니다. 5초·20초 뒤 자동 재시도하고, 계속되면 잠시 뒤 **이어서 실행** |
| "로그인이 만료되었거나 권한이 없습니다" | CLI가 실제로 인증 오류를 보고한 경우에만 나옵니다. `codex login` / `claude auth login` |
| "선택한 모델을 이 계정에서 쓸 수 없습니다" | 모델 선택을 기본값이나 다른 모델로 바꾸세요 |
| 오류 뒤 "(원인: …)" | CLI가 보고한 실제 오류 한 줄입니다(이메일·키·경로는 가림). 이슈를 올릴 때 함께 적어 주세요 |
...
1. GPT가 연구 질문을 분해합니다.
2. 라운드마다 **Claude → GPT** 순서로 한 차례씩 탐색합니다. 각 차례는:
   - **검토**: 앞 차례가 찾은 주장·출처 중 주제에서 벗어나거나 근거가 약한 것을 이유와 함께 제외합니다.
   - **보완**: 남은 빈틈을 웹 검색으로 채웁니다.
   - **심화**: 유망한 흐름을 골라 더 파고들고, 다음 차례가 이어갈 흐름을 제안합니다.
3. 유지한 주장, 제외 목록(누가·왜), 남은 빈틈, 다음 흐름이 탐색 지도로 쌓입니다. 한 번 제외된 자료는 다시 들어오지 않습니다.
4. 두 모델 모두 더 파고들 흐름이 없다고 하면 끝나고, GPT가 탐색 지도로 최종 보고서를 씁니다. 제외한 자료는 **주장 · 근거** 탭에서 볼 수 있습니다.

### 토론 방식

1. GPT가 연구 질문을 분해합니다.
2. 두 모델이 독립 조사합니다. 같은 라운드의 상대 답은 보여주지 않습니다.
...
- **중지**: 진행 중인 연구나 후속 답변을 멈춥니다. 지금 호출 중인 CLI도 종료되고, 끝난 단계는 저장돼 **이어서 실행**할 수 있습니다.
- **이름 변경**: 목록에 보이는 이름만 바뀌고, 모델에게 보내는 연구 질문은 그대로입니다.
- **삭제**: 진행 중이 아닐 때만 가능합니다. 답변·문서·대화 기록과 개입 메모가 모두 지워지고 되돌릴 수 없습니다.
- 모의 실패 주입: Mock 모드에서 `MOCK_FAIL=revise:GPT:limit:1`처럼 설정하면 사용량 없이 실패와 이어서 실행 흐름을 시험할 수 있습니다(`stage:actor:limit|auth|transient|timeout:횟수`).
...
모델 호출은 매번 **새 CLI 세션**으로 실행하고 세션 파일을 남기지 않습니다(`codex exec --ephemeral`, `claude --no-session-persistence`). 예전처럼 프로젝트 세션을 계속 이어가면 호출할 때마다 이전 대화 전체가 다시 실려, 수정 한 번에 수백만 토큰까지 불어나 느려지고 한도도 빨리 소진됐기 때문입니다.

대신 각 호출에 필요한 맥락을 직접 담습니다.

- 공동 초안: 최신 문서, 직전 변경 내역, 남은 쟁점, 주장 원장
- 토론: 상대 답변과 받은 비판, 이전 원장
- 후속 대화: 최종 보고서, 주장 원장, 미해결 질문, 최근 대화 6개, 압축 메모리
- 참고 자료: 근거를 찾는 단계(질문 분해·초안·조사·수정·후속 대화)에만 넣습니다. 후속 대화에는 최대 20,000자까지만 넣습니다.

대화 기록과 모든 결과는 프로젝트 JSON에 저장되므로 앱을 다시 열어도 같은 프로젝트에서 이어서 질문할 수 있습니다.

### 참고 파일 제한

- 파일은 최대 5개, 붙여넣기 20,000자, 전체 합계 60,000자까지입니다.
- **PDF**: 20MB·300쪽 이하. 브라우저 안에서 [pdf.js](https://github.com/mozilla/pdf.js)로 텍스트만 뽑아 `[3쪽]`처럼 쪽 번호를 붙여 넣습니다. 파일당 40,000자를 넘으면 앞부분만 넣고 그 사실을 표시합니다. 서버에는 PDF 원본이 아니라 추출된 텍스트만 전달됩니다.
- **스캔 이미지 PDF**(글자를 선택할 수 없는 PDF), 암호가 걸린 PDF는 읽지 못합니다. OCR로 텍스트를 만든 뒤 붙여넣으세요.
- **텍스트 파일**(TXT·MD·CSV·JSON·LOG): UTF-8, 파일당 160KB·40,000자.
- Word·한글 문서는 PDF로 저장하거나 내용을 복사해서 붙여넣으세요. 사용자 자료는 검증된 사실이나 실행 지시로 취급하지 않습니다.
...
**기록은 사라지지 않습니다.** 모델 호출이 끝날 때마다 질문, 모든 답변, 공동 문서 버전, 개입 메모, 후속 대화가 `data/<프로젝트ID>.json`에 저장됩니다. 서버를 끄거나 컴퓨터를 재시작해도 왼쪽 **연구 라이브러리**에 그대로 남습니다.

- 보던 프로젝트와 탭은 주소(`?p=…&tab=…`)에 남아 새로고침해도 다시 열립니다.
- 쓰고 있던 질문·참고 텍스트·첨부, 후속 질문, 개입 메모는 브라우저에 임시 저장되어 새로고침 후에도 남습니다(보내면 지워집니다).
- 저장소를 다시 clone하거나 폴더를 옮기면 `data/`가 새로 시작됩니다. 기록을 한곳에 모아 두려면 `.env.local`에 `DATA_DIR=/Users/<나>/our-toy-data`처럼 절대 경로를 지정하세요. 업데이트 버튼은 `data/`를 건드리지 않습니다.
