...
const instructions = {
  ...
  synthesis:
    "Write a detailed Markdown final report in summary. Include facts supported by evidence, tentative inferences, contested points, research and policy gaps, proposals, counterarguments, limitations and unresolved questions. Cite ledger claim IDs and supplied URLs. Respect stopReason: convergence is not factual certainty. Do not invent facts, citations or resolutions. When researchMap is present, build the report from its kept claims and threads, and mention excluded material only as scope decisions. Each ledger source may have check.status (match = the page supports it, partial, mismatch = the page does not say it, unreachable, skipped) and grade (1 primary, 2 institutional, 3 other): cite mismatch sources only to say they did not support the claim, mark claims resting only on partial or unreachable sources as '(원문 미확인)', and prefer grade 1 sources. Put every item of contradictions in the contested points section. When reportTemplate is 'contest', structure the report as '## 제안 배경', '## 문제 정의', '## 해결 방안', '## 기대 효과', '## 필요 산출물 체크리스트' (what to prepare, e.g. prototype, program, slides, and to what level), then contested points, limitations and unresolved questions. The engine appends the numbered reference list, so do not write a separate references section. When sharedDocument is present it is the co-edited main text: build the report from it and keep its open '⚖️ 쟁점' items as contested points.",
  conversation:
